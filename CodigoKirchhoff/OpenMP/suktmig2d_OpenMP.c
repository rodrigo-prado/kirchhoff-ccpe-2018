...
#pragma omp parallel firstprivate(k,imp,iip,it,ifc,p,pmin,x,xmax,obliq,geoms,\
mp,ip,t,t0,ang,angtaper,v,filter,lowpass,rtin,rtout,ct,datalo,datahi,itb, ite,\
firstt, amplo, amphi)
{
//warn("Aviso # A # = %d",omp_get_thread_num());
...
/*
    warn("Aviso # d # = %d %p lowpass",omp_get_thread_num(),&lowpass);
    warn("Aviso # d # = %d %p rtin",omp_get_thread_num(),&rtin);
    warn("Aviso # d # = %d %p rtout",omp_get_thread_num(),&rtout);
    warn("Aviso # d # = %d %p ct",omp_get_thread_num(),&ct);
    warn("Aviso # d # = %d %p filter",omp_get_thread_num(),&filter);
*/
	// Zero all arrays /
	memset((void *) mig[0], 	0, nt*ntr*FSIZE);
    ...
    //rtin[0] = 0.0;
    //warn("Aviso # e # = %d",omp_get_thread_num());
...
/*
#pragma omp for private(k,imp,iip,it,ifc,p,pmin,x,xmax,obliq,geoms,\
mp,ip,t,t0,ang,angtaper,v,filter,lowpass,rtin,rtout,ct,datalo,datahi,itb, ite,\
firstt, amplo, amphi)
	for(imp=0; imp<ntr; ++imp){
		float perc;
