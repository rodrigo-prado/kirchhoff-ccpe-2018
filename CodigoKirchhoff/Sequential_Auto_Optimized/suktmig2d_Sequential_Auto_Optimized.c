...
void lpfilt(int nfc, int nfft, float dt, float fhi, float * restrict filter);

void migkt2d (const float * const *restrict data,const int  ntr,const int  nt,const float  dx,const float  dt,const float  tmax, 
              const int  nfft,const float  fnyq,const float  h,const float *restrict fc,const int  nf,const int  nc,const int  nfc, 
              const int  cdp_trace_first,const float  angmax,const float * const*restrict vel, float ** restrict mig,const int  dxcdp,const int  firstcdp);    
...
/*		if(fmod(imp*100,ntr-1)==0)
		...
		for(it=0; it<nt; ++it){
			...
			//v=vel[imp*(dxcdp-1)+cdp_trace_first-1][it];
...
void
lpfilt(int nfc, int nfft, float dt, float fhi, float * restrict filter)
/*******************************************************************************
